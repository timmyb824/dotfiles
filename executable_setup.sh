...
install_chezmoi() {
    echo_with_color "$GREEN_COLOR" "Installing chezmoi..."
    if command_exists chezmoi; then
        echo_with_color "$YELLOW_COLOR" "chezmoi is already installed."
        return 0
    fi
...
make_package_script_executable() {
    local script=$1
    if [ -f "$script" ] && [ ! -x "$script" ]; then
        echo_with_color "$BLUE_COLOR" "Making $(basename "$script") executable..."
        chmod +x "$script"
    fi
}
...
run_setup_scripts() {
    local script_path="$SCRIPT_DIR/$1"
    if [ -f "$script_path" ]; then
        echo_with_color "$GREEN_COLOR" "Running $1..."
        make_package_script_executable "$script_path"
        "$script_path" || exit_with_error "$1 failed to complete."
        echo_with_color "$GREEN_COLOR" "$1 completed."
    else
...
package_installation() {
    if ask_yes_or_no "Do you want to install the packages?"; then
        make_package_script_executable "$INSTALL_PACKAGES_SCRIPT"
        echo_with_color "$GREEN_COLOR" "Running package installation script."
        "$INSTALL_PACKAGES_SCRIPT" || exit_with_error "Failed to execute package installation."
    else
        echo "Package installation skipped."
    fi
...
if [[ "$OS" == "MacOS" ]]; then
    if ask_yes_or_no "Do you want to remove the chezmoi binary (if installing later with homebrew)?"; then
        echo_with_color "$BLUE_COLOR" "Removing binary..."
        safe_remove_command "$CHEZMOI_BIN"
        # safe_remove_command "/usr/local/bin/sops" # In the future, sops will be installed via pkgx
        # safe_remove_command "/usr/local/bin/age" # In the future, age will be installed via pkgx
    else
        echo_with_color "$BLUE_COLOR" "Skipping binary removal."
    fi
else
